...
function sendMessage() {
    ...
    if ($("#mobileCheckbox").get(0).checked || $("#unicomCheckbox").get(0).checked ||
        $("#telecomCheckbox").get(0).checked || $("#invalidCheckbox").get(0).checked) {
        isCheckBox = true;
    }
    ...
    if (isEmpty) {
        ...
        var path = $("#path").val();
        var forwardUUID = $("#forwardUuid").val();
        var operators = operators.toString().substring(1);
        //window.location.href = path + '/smsUser/sendView.do?forwardUuid=' + $("#forwardUuid").val() + "&operators=" + operators.toString().substring(1);
        window.location.href = path + '/smsFilter/toSendView.do?forwardUuid=' + forwardUUID + "&operators=" + operators;
...
function exportMessage() {
    ...
    if (($("#mobileCheckbox").get(0).checked && $("#mobile").val() == "")
        || ($("#unicomCheckbox").get(0).checked && $("#unicom").val() == "")
        || ($("#telecomCheckbox").get(0).checked && $("#telecom").val() == "")
        || ($("#invalidCheckbox").get(0).checked && $("#invalid").val() == "")) {
        alert("空号码无法导出，请选择非空号码进行导出！");
        return;
    }
...
function jquploadFile() {
    if (navigator.appName == "Microsoft Internet Explorer" && navigator.appVersion.match(/9./i) == "9." ||
        ...
        alert("当前浏览器版本过低，不支持上传功能，请升级浏览器。")
        return;
    }
    var uuid = $("#uuid").val();
...
$('#upload_file').on('show.bs.modal', function (e) {
    ...
    var modalHeight = $(window).height() / 2 - $('#upload_file .modal-dialog').height() / 2 - 30;
