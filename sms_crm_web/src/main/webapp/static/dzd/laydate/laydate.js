...
! function(a) {
	...
	a.laydate = function(b) {
		...
		if(c.options.up){
			...
			d =	 e.top - a.offsetHeight + 1 , a.style.top = d + (b ? 0 : c.scroll()) + "px"
		}else{
		...
		}() + "</div>" + "</div>" + "</div>" + c.viewtb + '<div class="laydate_bottom">' + '<ul id="laydate_hms">' + '<li class="laydate_sj">时间</li>' + "<li><input readonly>:</li>" + "<li><input readonly></li>" + "<li class='laydatesecend'>:<input readonly></li>" + "</ul>" + '<div class="laydate_time" id="laydate_time"></div>' + '<div class="laydate_btn">' + '<a id="laydate_clear">清空</a>' + '<a id="laydate_today">今天</a>' + '<a id="laydate_ok">确认</a>' + "</div>" + (b.isv ? '<a href="http://sentsin.com/layui/laydate/" class="laydate_v" target="_blank">laydate-v' + laydate.v + "</a>" : "") + "</div>", d.body.appendChild(i), c.box = g("#" + h[0]), c.events(), i = null), c.follow(c.box), f.zIndex ? c.box.style.zIndex = f.zIndex : c.removeCssAttr(c.box, "z-index"), c.stopMosup("click", c.box), c.initDate(), c.iswrite(), c.check()
	}, c.reshow = function() {
		return c.each(c.query("#" + h[0] + " .laydate_show"), function(a, b) {
			c.removeClass(b, "laydate_show")
		}), this
	}, c.close = function(dali) {
		var dali=dali;	
		if(dali){
			c.reshow(), c.shde(c.query("#" + h[0]), 1,dali), c.elem = null
		}else{
	...
	}, c.creation = function(a, b,day) {
		var day=day;		
		if(day){		
			var e = (c.query, c.hmsin),
			f = c.parse(a, [e[0].value, e[1].value, e[2].value]);
		c.elem[h.elemv] = f, b || (c.close(day), "function" == typeof c.options.choose && c.options.choose(f))
		}else{
			var e = (c.query, c.hmsin),
			f = c.parse(a, [e[0].value, e[1].value, e[2].value]);
		...
		c.each(b("#laydate_table td"), function(a, gg) {
			c.on(gg, "click", function(a) {					
				var dayon=c.options.dayon;				
				if(dayon){				
					c.hasClass(this, h[1]) || (c.stopmp(a), c.creation([0 | this.getAttribute("y"), 0 | this.getAttribute("m"), 0 | this.getAttribute("d")],gg,dayon))									
					if(new RegExp("laydate_void").test(this.className)){						
						cilcky=c.ymd[0],
						cilckm=c.ymd[1] + 1,
						cilckd= c.ymd[2]  
					}else{						
						cilcky=0 | this.getAttribute("y");
						cilckm=0 | this.getAttribute("m");
						cilckd=0 | this.getAttribute("d");
						$("#laydate_table td").removeClass("laydate_click");
			...
			if(dayon){
				var d=cilcky;
				if(cilcky==""||cilckm==""||cilckd==""){					
					c.valid && c.creation([c.ymd[0], c.ymd[1] + 1, c.ymd[2]])
				}else{
					c.valid && c.creation([cilcky, cilckm , cilckd])
				}
