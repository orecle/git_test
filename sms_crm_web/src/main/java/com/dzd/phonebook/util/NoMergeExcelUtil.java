...
public class NoMergeExcelUtil
{
	...
	public static void reportMergeTxt(HttpServletRequest request, HttpServletResponse response,
	        List<String> dataList) throws Exception
	{
		String fileName = new String(SessionUtils.getSmsUser(request).getEmail() + "-"
		        + new SimpleDateFormat("yyyyMMdd-HHmmss").format(new Date()));
		// 设置response参数，可以打开下载页面
		response.reset();
		response.setContentType("applicationnd.ms-excel;charset=utf-8");
		response.setHeader("Content-Disposition", "attachment;filename=\""
		        + new String(fileName.getBytes("gb2312"), "ISO8859-1") + ".txt" + "\"");
		...
		try
		{
            ...
            buffOut = new BufferedOutputStream(outSTr);  
            StringBuffer write = new StringBuffer(); 
            
            for (int i = 0; i < dataList.size(); i++) {  
                write.append(dataList.get(i));  
                write.append(enter);          
            }  
            buffOut.write(write.toString().getBytes("UTF-8"));  
		} catch (final IOException e)
		{
			throw e;
		} finally
